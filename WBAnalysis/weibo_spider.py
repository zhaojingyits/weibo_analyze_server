...
from . import user_serializer,entity_serializer

class Weibo(object):
    def __init__(self, config, user_id_list_from_arguments):
        """Weibo类初始化"""
        ...
        self.write_mode = config[
            'write_mode']  # 结果信息保存类型，为list形式，可包含django_model、mongo和mysql三种类型
        ...
        user_id_list = user_id_list_from_arguments
    ...
    def validate_config(self, config):
        ...
        # 验证write_mode
        write_mode = ['django_model', 'json']
        if not isinstance(config['write_mode'], list):
            sys.exit(u'write_mode值应为list类型')
        for mode in config['write_mode']:
            if mode not in write_mode:
                sys.exit(
                    u'%s为无效模式，请从django_model、json、mongo和mysql中挑选一个或多个作为write_mode' %
                    mode)
    ...
    def user_to_django_model(self):
        print(self.user)
        userdata=user_serializer.WeiboUserSerializer(data=self.user)
    ...
    def write_django_model(self, wrote_count):
        """将爬到的信息写入django_model文件"""
        write_infos = self.get_write_info(wrote_count)
        for write_info in write_infos:
            write_info['user_id']=self.user.get('id')
            write_info['screen_name']=self.user.get('screen_name')
            print(write_info)
            print("------------------")
            entitydata=entity_serializer.WeiboEntitySerializer(data=write_info)
            # 写入原entity没有包含的用户信息 结束
            if entitydata.is_valid()== False:
                print(entitydata.errors)
            else:
                print(entitydata.validated_data)
                u=entitydata.save()
                print('成功存数据')
    ...
    def write_data(self, wrote_count):
        ...
        if self.got_count > wrote_count:
            if 'django_model' in self.write_mode:
                self.write_django_model(wrote_count)
    ...
    def get_pages(self):
        ...
        try:
            ...
            if since_date <= today:
                ...
                for page in tqdm(range(1, 6), desc='Progress'):
                    # 6是设置的页数限制，如取消限制应为page_count + 1
        ...
        with open(config_path) as f:
            try:
                ...
                sys.exit(u'config.json 格式不正确，请参考 '
                         u'https://github.com/dataabc/weibo-crawler#3程序设置')
        wb = Weibo(config,target_id_list)
        wb.start()  # 爬取微博信息
    except Exception as e:
        print('Error: ', e)
        traceback.print_exc()
